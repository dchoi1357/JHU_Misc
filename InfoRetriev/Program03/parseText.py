	...
	with open (fname, 'r') as f:
		for line in f: # NOTE: must read file line by line due to large size
			if not inDoc: # before start of a document
				docID = int( tg.findall(line)[0] ) # assume 1st match is right
				inDoc = True
				tmpTxt = '' # accumulate text for processing by document
			else: # in the middle of parsing a document
				if line.strip().casefold() != '</p>': # not end of document
					tmpTxt += line.casefold()
				else:
					inDoc = False # set to no longer in document
					tmpDict = processDoc(tmpTxt)
